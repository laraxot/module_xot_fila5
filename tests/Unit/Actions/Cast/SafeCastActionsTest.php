...
test('safe array cast action works', function () {
    ...
    expect($action->execute(['a' => 1]))->toBe(['a' => 1])
        ...
        ->and($action->execute(new class { public $d = 4; }))->toBe(['d' => 4])
        ->and($action->execute(new class { public function toArray() { return ['e' => 5]; } }))->toBe(['e' => 5])
        ->and($action->execute(new class { public function __toArray() { return ['f' => 6]; } }))->toBe(['f' => 6]);
...
test('safe int cast action works', function () {
    ...
    expect($action->execute(123))->toBe(123)
        ...
        ->and($action->execute(new class { public function __toString() { return '1011'; } }))->toBe(1011)
