...
created: 2026-05-05
updated: 2026-05-05
tags: [filament, wizard, haswizard, architecture, trait]
related:
  - concepts/filament-wizard-zen.md
  - ../../../../docs/wiki/concepts/filament-haswizard-vs-xotbasewizard.md
---

# Filament HasWizard Traits: Study Notes
...
**Key insight:** Uses `$this->schema($steps)` (not `$this->form()`)
...
**Key insight:** Uses `$this->form($schema)->components([$this->getWizardComponent()])`
...
### Current Problem (545 lines)
`XotBaseWizardWidget` **reinvents** what `Wizard` component + `HasWizard` traits already do:
...
{
    // D ON'T reinvent - use Filament's Wizard component properly
    // Keep Xot-specific additions ONLY:
