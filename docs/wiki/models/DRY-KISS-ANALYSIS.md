...
- SurveyModule: ❌ **ECCEZIONE** - Non estende XotBaseModel (da correggere)
- UI: Vuoto (minimal)
...
### 4. **SurveyModule BaseModel - Pattern Anomalo**

**Problema Critico**: `Modules\SurveyModule\Models\BaseModel` NON estende `XotBaseModel`:
...
// SurveyModule/BaseModel duplica Updater:
...
### Soluzione 2: Correggere SurveyModule/BaseModel

**Obiettivo**: Allineare SurveyModule all'architettura standard

**Implementazione**:
```php
// Modules/SurveyModule/app/Models/BaseModel.php
...
abstract class BaseModel extends XotBaseModel implements HasMedia {
    ...
    protected $connection = 'survey_module'; // Auto-discovery se Soluzione 1 applicata

    protected $with = ['extra']; // Specifico SurveyModule

    protected function casts(): array {
        return array_merge(parent::casts(), [
            // Solo casts SPECIFICI SurveyModule, se necessari
        ]);
    }
}
```
...
### 🔴 Priorità ALTA
1. **Correggere SurveyModule/BaseModel** (non segue standard)
...
### Fase 2: Correzione SurveyModule
- `Modules/SurveyModule/app/Models/BaseModel.php` (refactor completo)
...
- `Modules/SurveyModule/docs/models/README.md`

## ✅ Checklist Implementazione

- [ ] Implementare `getConnectionName()` in XotBaseModel
- [ ] Testare auto-discovery con modello test
- [ ] Correggere `Modules/SurveyModule/app/Models/BaseModel.php`
- [ ] Testare modelli SurveyModule con nuova struttura
...
2. **Consistenza è chiave**: SurveyModule devia dallo standard → maggiore complessità
