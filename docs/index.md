...
- **Test Coverage**: 179 tests as of 2026-09-17 (168 unit + 11 feature; verified via `find tests/Unit|Feature -name '*Test.php'` — the "97 unit" figure below was stale, the suite has grown)
...
**Module Stats** (re-verified 2026-09-17):
- Models: 45 (`app/Models/**/*.php`, confirmed accurate)
- Test Files: 179 (168 unit + 11 feature, up from the 108 recorded 2026-06-11)
- Documentation Files: this `docs/` tree has 6800+ `.md` files, not 1957 — see the dedicated
  finding on duplication/conflict-marker corruption below and in the follow-up notes
...
├── routes/
├── tests/
│   ├── Unit/           # 168 unit tests (2026-09-17)
...
### Coverage Status
**Current**: 179 tests (verified 2026-09-17)
...
- `tests/Unit/` - Core functionality, models, services
  - 168 unit test files covering models, actions, services (2026-09-17)
...
### Knowledge Base
- **llm-wiki/** — knowledge base subfolder (8 files as of 2026-09-17, not the 1957 previously
  claimed here — see [wiki/index.md](./wiki/index.md) for the actively maintained wiki, which
  is a different, much larger directory)
  - Architecture guides
  - Service patterns
...
### Internal References
- [BMAD Workflow](../../../../docs/wiki/bmad/workflow.md) (path fixed 2026-09-17; was missing three `../`)
- [Xot Wiki](./wiki/index.md) — 993 files, the actual actively-maintained knowledge base
- [Xot llm-wiki/](./llm-wiki/) — smaller, separate subfolder (8 files)

Removed 2026-09-17 as dead links with no plausible target anywhere in this repo: "Base
Module Guide" (`docs/wiki/modules/base-module-guide.md`) and "Testing Standards"
(`docs/wiki/testing/standards.md`).

---

## Contributing

For contribution guidelines, see [CONTRIBUTING.md](../CONTRIBUTING.md) (path fixed
2026-09-17: was `../../CONTRIBUTING.md`, pointing outside the module; the file lives at
`Modules/Xot/CONTRIBUTING.md`)

When contributing to Xot:
1. Maintain test coverage (179 tests as of 2026-09-17)
2. Follow patterns established in core models
3. Update CHANGELOG.md
4. Document in wiki/ (or llm-wiki/) if adding new patterns

---

**Module Stats Summary** (re-verified 2026-09-17)
- 45 Models (thoroughly documented)
- 179 Tests (168 unit + 11 feature)
- 6800+ files in `docs/`, of which ~450 still contain unresolved Git conflict markers
  (see the dedicated finding — not something this pass could fix in full)
...
- [Development Philosophy](./philosophy/xot-philosophy.md) — forward-only, DRY, KISS, Zen
  (link corretto 2026-09-17: puntava a `philosophy/index.md`, mai esistito; `philosophy/`
  non ha un index, il file più vicino per contenuto è `xot-philosophy.md`).
...
- [No Label Rule](../../UI/docs/filament/no-label-rule.md) — regola "no explicit label" (link
  corretto 2026-09-17: puntava a `UI/docs/autolabel-guidelines.md`, mai esistito).
...
(Rimossi 2026-09-17: "Fixcity Module" e "Predict Module" — nessuno dei due moduli esiste in
questo repo/prodotto, `Modules/Fixcity` e `Modules/Predict` non ci sono; probabilmente
copiati da un'altra istanza del monorepo Laraxot che li include.)
...
Nota 2026-09-17: i tre link sotto sono rotti — `filament/`, `architecture/` e `phpstan/`
esistono come directory ma nessuna delle tre contiene un `index.md`; date le centinaia di
varianti/duplicati in quelle cartelle (es. 300+ file `phpstan-*.md`), non è stato scelto un
sostituto specifico per non puntare per errore a un file dal contenuto diverso. Usa
`bashscripts/docs/llm-wiki-qmd.sh search "<topic>"` o `rg` dentro quelle cartelle finché non
esiste un indice reale.
...
- 🧬 [Trait Patterns](./traits-complete-guide.md) — HasTeams, HasXotTable e altri trait core
  (link rotto, nessun file con questo nome esiste; nessun sostituto ovvio, non corretto).
...
Nota 2026-09-17: alcuni link in questa sezione storica sono rotti (i file non esistono più,
o esistono solo come uno tra molti nomi-varianti quasi identici — vedi il finding dedicato
sulla duplicazione/corruzione della cartella `docs/`). Non corretti singolarmente per non
scegliere a caso il file sbagliato tra i duplicati; segnalati qui invece che silenziati:
`architecture-complete-2025.md`, `phpstan-analysis-2025-01-27.md`,
`phpstan-analysis-2025-12-17.md`, `phpstan-analysis-2025-12-18.md`,
`quality-improvements-summary-2025-11-18.md`.
