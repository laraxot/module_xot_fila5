...
## Overview
MCP (Model Context Protocol) tools provide enhanced capabilities for database analysis, including access to the survey_module_survey database used in the Limesurvey integration.
...
**Use Cases for survey_module_survey Database**:
...
```sql
-- List all survey tables in survey_module_survey database
...
Current configuration includes MySQL access that automatically uses Laravel's .env credentials, making it ideal for accessing the survey_module_survey database without additional configuration.

## Best Practices for Database Analysis

1. **Always verify survey IDs** before querying dynamic tables like `lime_survey_{id}`
2. **Use proper connection** (`limesurvey` connection for survey_module_survey database)
