...
1. **Accoppiamento Stretto**: Cms conosce Application → viola principio di disaccoppiamento
...
'providers' => [
    'users' => [
        'driver' => 'eloquent',
        'model' => \Modules\Application\Models\User::class, // CONFIGURABILE!
    ],
],
```
...
public function processUser(\Modules\Application\Models\User $user): void
```
...
// ❌ VIETATO
use Modules\Application\Models\User;
use Modules\Application\Models\Patient;
use Modules\Application\Models\Doctor;
...
```php
// ❌ VIETATO
$user = \Modules\Application\Models\User::find($id);
...
```php
// ❌ VIETATO
function updateUser(\Modules\Application\Models\User $user): void
...
- [Testing Strategy](../../Application/project_docs/testing/real-data-testing-strategy.md)
