...
public function submit(): void
{
    try {
        ...
        $message = (string) __('application::create_ticket_wizard.notifications.submit_failed.body');
        $this->addError('data.submit', $message);
        
        Notification::make()
            ->title((string) __('application::create_ticket_wizard.notifications.submit_failed.title'))
            ->body($message)
            ->danger()
            ->send();
    }
